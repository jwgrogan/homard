...
pub fn store_secret(service: &str, account: &str, secret: &str) -> Result<()> {
    ...
    set_generic_password(service, account, secret.as_bytes())
        .map_err(|e| ArcctlError::Keychain(e.to_string()))?;
    Ok(())
}
    ...
    match get_generic_password(service, account) {
        Ok(bytes) => {
            let s = String::from_utf8(bytes)
                .map_err(|e| ArcctlError::Keychain(format!("UTF-8 decode error: {}", e)))?;
            Ok(Some(s))
        }
        Err(e) => {
...
mod tests {
    ...
    fn test_keychain_roundtrip() {
        let service = "arcctl-test-keychain";
    ...
    fn test_keychain_missing_returns_none() {
        let result = read_secret("arcctl-test-keychain", "definitely-does-not-exist-xyz").unwrap();
