...
pub fn generate_plist(schedule: &Schedule, homard_bin: &str) -> String {
    let label = format!("com.homard.job.{}", schedule.id);
    let home = dirs::home_dir().unwrap_or_else(|| PathBuf::from("/tmp"));
    let log_dir = home.join(".homard").join("logs");
...
pub fn install_plist(schedule: &Schedule, homard_bin: &str) -> crate::Result<PathBuf> {
    let home = dirs::home_dir().ok_or_else(|| {
        crate::HomardError::Io(std::io::Error::new(
            std::io::ErrorKind::NotFound,
            "home directory not found",
        ))
    })?;
    let agents_dir = home.join("Library").join("LaunchAgents");
    std::fs::create_dir_all(&agents_dir).map_err(crate::HomardError::Io)?;

    let label = format!("com.homard.job.{}", schedule.id);
    let plist_path = agents_dir.join(format!("{}.plist", label));
    let content = generate_plist(schedule, homard_bin);
    std::fs::write(&plist_path, &content).map_err(crate::HomardError::Io)?;
...
pub fn uninstall_plist(schedule_id: &str) -> crate::Result<()> {
    ...
    let label = format!("com.homard.job.{}", schedule_id);
    let plist_path = home
        .join("Library")
        .join("LaunchAgents")
    ...
    if plist_path.exists() {
        let _ = std::process::Command::new("launchctl")
            ...
            .arg(&plist_path)
            .output();
        std::fs::remove_file(&plist_path).map_err(crate::HomardError::Io)?;
    }
...
pub fn scan_claude_plists(dir: &Path) -> crate::Result<Vec<DiscoveredPlist>> {
    ...
    let entries = std::fs::read_dir(dir).map_err(crate::HomardError::Io)?;
...
pub fn parse_simple_cron(cron: &str) -> std::result::Result<(Option<u32>, Option<u32>), String> {
...
mod tests {
    ...
    fn test_generate_plist() {
        ...
        let plist = generate_plist(&schedule, "/usr/local/bin/homard");

        assert!(plist.contains("com.homard.job.job-123"));
